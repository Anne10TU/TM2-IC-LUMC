...
def select_occlusion():
    ...
    def send_occlusion(occlusie_type, severity):
        ...
        if occlusie_type == "arterieel":
            set_scenario(f"occlusie_art-{severity}", 40, 90, None)  # Voorbeeld waarden voor arterieel
        elif occlusie_type == "veneus":
            set_scenario(f"occlusie_ven-{severity}", 95, 50, None)  # Voorbeeld waarden voor veneus
...
# Scenario-knoppen
scenarios = {
    "Hypertensie": lambda: set_scenario("Hypertensie", 70, 90, hypertensie_button),
    "Hypotensie": lambda: set_scenario("Hypotensie", 95, 62, hypotensie_button),
    "Occlusie": select_occlusion,  # Enkele knop voor "Occlusie"
    "Stabiel": lambda: set_scenario("Stabiel", 95, 90, stabiel_button),
}

# Maak knoppen en voeg ze toe aan de GUI
scenario_buttons = []
for name, action in scenarios.items():
    button = tk.Button(button_frame, text=name, command=action, font=("Arial", 14), width=20, height=2)
    button.pack(pady=5)
