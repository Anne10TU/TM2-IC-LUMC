...
def scenario_button(scenario, art, ven):
    ...
    current_scenario_var.set(f"Huidig Scenario: {scenario.capitalize()}")  # Toon het geselecteerde scenario
...
# Handmatige regeling (onder de knoppen)
slider_frame = tk.LabelFrame(root, text="Handmatige regeling", font=("Arial", 12))
...
tk.Scale(slider_frame, from_=0, to=100, variable=arterial_dc,
         orient=tk.HORIZONTAL, length=350, command=manual_adjust).grid(row=0, column=1)

tk.Label(slider_frame, text="Veneus DC").grid(row=1, column=0, sticky="w")
tk.Scale(slider_frame, from_=0, to=100, variable=venous_dc,
         orient=tk.HORIZONTAL, length=350, command=manual_adjust).grid(row=1, column=1)
...
# Huidig scenario label
current_scenario_var = tk.StringVar(value="Huidig Scenario: Geen")
tk.Label(bottom_frame, textvariable=current_scenario_var, font=("Arial", 12)).pack(pady=5)
