# -*- coding: utf-8 -*-
"""
Created on Tue Apr 15 16:03:40 2025
    ...
    if ser:
        try:
            while ser.in_waiting > 0:
                ...
                elif message == "RUNNING":  # Markeer de Arduino als bezig
                    is_ready = False
...
def set_scenario(scenario_name, dc_art, dc_ven, button):
    ...
    # Reset de kleur van andere knoppen en markeer de geselecteerde knop
    reset_button_colors()
    button.config(bg="blue", fg="white")  # Maak de geselecteerde knop blauw


def reset_button_colors():
    """Reset de kleur van alle scenario knoppen naar de standaardkleur."""
...
def select_occlusion():
    ...
    def send_occlusion(occlusie_type, severity):
        ...
        if occlusie_type == "arterieel":
            set_scenario(f"occlusie_art-{severity}", 40, 90, arterieel_occlusie_button)  # Voorbeeld waarden voor arterieel
        elif occlusie_type == "veneus":
            set_scenario(f"occlusie_ven-{severity}", 95, 50, veneus_occlusie_button)  # Voorbeeld waarden voor veneus
...
# Scenario-knoppen
hypertensie_button = tk.Button(button_frame, text="Hypertensie", command=lambda: set_scenario("Hypertensie", 70, 90, hypertensie_button), font=("Arial", 14), width=20, height=2)
hypotensie_button = tk.Button(button_frame, text="Hypotensie", command=lambda: set_scenario("Hypotensie", 95, 62, hypotensie_button), font=("Arial", 14), width=20, height=2)
occlusie_button = tk.Button(button_frame, text="Occlusie", command=select_occlusion, font=("Arial", 14), width=20, height=2)
stabiel_button = tk.Button(button_frame, text="Stabiel", command=lambda: set_scenario("Stabiel", 95, 90, stabiel_button), font=("Arial", 14), width=20, height=2)

# Voeg knoppen toe aan de lijst
scenario_buttons = [hypertensie_button, hypotensie_button, occlusie_button, stabiel_button]

# Voeg knoppen toe aan de GUI
for button in scenario_buttons:
    button.pack(pady=5)
